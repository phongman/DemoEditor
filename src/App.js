...
function App() {
  ...
  // const chars = new antlr4.InputStream(lineQuery);
  // const lexer = new NqlLexer(chars);
  // const tokens = new antlr4.CommonTokenStream(lexer);
  // const parser = new NqlParser(tokens);
  // const [states, setStates] = useState(parser.atn.states);
  ...
  function handleEditorDidMount(_, editor) {
    ...
    editor.onKeyUp(ev => {
      ...
      setLineQuery(model.getLineContent(position.lineNumber).slice(0, position.column - 1))
      setCaret(position)
    })
  }
  ...
  const describeTransition = (t) => {
    ...
    if (t instanceof SetTransition){
      return getLabel(t.label.intervals).map(el => NqlLexer.symbolicNames[el]).join(', ')
    }
  ...
  const describeState = (s) => {
    ...
    if (s instanceof BasicBlockStartState)
      return `basic block start`
    if (s instanceof BlockEndState) 
      return `block end (start ${s.startState})`
  ...
  const isCompatibleWithStack = (state, parserStack) => {
    ...
    if(state.epsilonOnlyTransitions) {
      for(let t of state.transitions) {
        if(isCompatibleWithStack(t.target, res.states))
          return true;
      }
      return false;
    }
  ...
  const autoCompleteProcess = (state, tokens, collector, parserStack, alreadyPassed ,history) => {
    ...
    for(let transition of state.transitions) {
      ...
      if (transition.isEpsilon) {
        ...
        if (!alreadyPassed.includes(stateNumber)){
          ...
          autoCompleteProcess(target, tokens, collector, stackRes.states, _alreadyPassed, history)
        }
          ...
          if (atCaret) {
            ...
            if (nextTokenType.type === label) {
              autoCompleteProcess(target, tokens.move(), collector, stackRes.states, [], history)
            }
          }
  ...
  const preceedingTokens = (input) => {
    ...
    do {
      ...
      if (next.channel === 0) {
        if (next.type < 0)
          next = new CommonToken(undefined, CARET_TOKEN_TYPE);
        res.push(next)
      }
    } while (next.type >= 0);
    return res
  ...
  const suggest = (input) => {
    ...
    autoCompleteProcess(ATN_FIRST_STATE, new TokenStream(tokens), collector, new ParserStack(), [], history)
  ...
  const normalize = (tokenType) => {
    ...
    if (displayName.startsWith("'") && displayName.endsWith("'")) 
      return displayName.substring(1, displayName.length() - 1).replaceAll("\\'","'")
    return displayName
  }
  ...
  console.log(predict(fields,functions,lineQuery))  
